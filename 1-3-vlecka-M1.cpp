...
struct Vagon vytvorVagon(int identifikator) 
{
	struct Vagon vagon; //vytvori vagon
	vagon.identifikator=identifikator; //nastavi mu identifikaci z parametru teto funkce
	...
	return vagon; //vrati vygenerovany vagon
}
...
void naplnVlak(Zasobnik* vlak)
{
	for (unsigned int i=1;i<=M;++i) {// cyklus jde od 1 do M (vygeneruje se behem nej tedy M vagonu a ty se priradi k vlaku)
		Vagon vagon=vytvorVagon(i); //vygeneruje se M vagonu s id od 1 do M
...
void odstavVagony(Zasobnik* vlak,Zasobnik* koleje)
{
	while (!jePrazny(vlak)) { //dokud neni vlak prazdny
		struct Vagon vagon=odeber(vlak); //odeber z nej vagon a uloz jej do docasne promenne vagon
...
void pripojVagony(Zasobnik* vlak,Zasobnik* koleje)
{
	for (unsigned int i=0;i<N;++i) { // ze vsech kolejnic od první (koleje[0]) po poslední (koleje[N-1])
		while(!jePrazny(&koleje[i])) {  // dokud neni dana kolejnice prazdna
			struct Vagon vagon=odeber(&koleje[i]); //odeber z kolejnice vagon a uloz jej do docasne promenne vagon
