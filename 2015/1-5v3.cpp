...
Fronta* FrontaCreate(int cislo) // Vytvori novou frontu
{
	...
	return fronta; // vrati adresu fronty
}
...
Seznam* SeznamCreate(void) //Vytvori novy seznam
{
	Seznam *seznam = new Seznam; // vytvori seznam
	...
	return seznam; // vrati adresu seznamu
}
...
void DealokaceSeznamu(Seznam *seznam)
{
	...
	delete seznam; // smaze dynamicky alokovany seznam
}
...
Fronta* PridatFrontu(Seznam *seznam)
{
	...
	Fronta *fronta = FrontaCreate(seznam->pocet_front); // vytvori novou frontu s cislem seznam->pocet_front

	if (!konec) { // pokud v seznamu dosud zadna fronta nebyla
		seznam->zacatek = fronta; // nastav zacatek seznamu na tuto (nove vytvorenou) frontu
		return fronta; // vrat tuhle frontu
	}

	while (konec->dalsi) { // dokud neukazuje konec na posledni frontu v seznamu
...
Fronta* VratitNeplnouFrontu(Seznam *seznam)
{
	...
	if (!ukazatel) { // pokud seznam neobsahuje zatim zadnou frontu (je prazdny)
...
bool JsouFrontyPrazdne(Seznam *seznam)
{
	...
	if (!ukazatel) { // pokud je ukazatel == NULL, tak je seznam prazdny
