...
Balik frontaPodivat(Fronta *fronta) // funkce vrati prvni prvek z fronty, ale neodebere jej
{
	pFronta *ukazatel = fronta->konec_fronty; // nastavi ukazatel na konec fronty

	while(ukazatel->dalsi != NULL)  //dokud nemam ukazatel nastaven na prvni prvek ve fronte
		ukazatel = ukazatel->dalsi; // posun ukazatel dopredu

	return ukazatel->balik; //vrat balik
...
int main(void)
{
	...
	while(!jsouPasyPrazdne(pasy)) // dokud nejsou vsechny pasy prazdne
	{
		...
		{
			if (frontaJePrazdna(&pasy[i])) //pokud je pas prazdny, nic nedelej a skoc o jednu iteraci v cyklu dal
				continue;

			balik = frontaPodivat(&pasy[i]); // jinak uloz balik z pasu do promenne balik (zatim se neodebira)
			int m = balik.id % M; // zjisti podle ID tohoto baliku, do ktereho kontejneru se ma vlozit
			cout << "P" << m << "[" <<kontejner[m].poradove_cislo << "]" << endl; //vypis info o pripojeni tohoto kontejneru
